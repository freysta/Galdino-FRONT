...
export default function OnibusPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<BusStatus | null>(null);
  ...
  const {
    ...
    isLoading,
    error,
  } = useBuses(undefined, statusFilter || undefined);
  ...
  const totalCapacity = busesArray.reduce(
    (sum: number, b: Bus) => sum + (b.capacidade || 0),
    0,
  );
  ...
  return (
    <Stack gap="lg">
        ...
        <Grid>
          ...
          <Grid.Col span={{ base: 12, md: 3 }}>
            <Select
              ...
              value={statusFilter}
              onChange={(value) => setStatusFilter(value as BusStatus)}
              clearable
            />
          </Grid.Col>
          <Grid.Col span={{ base: 12, md: 5 }}>
            <Text size="sm" c="dimmed" ta="right">
              {filteredBuses.length} registro(s) encontrado(s)
            </Text>
          </Grid.Col>
        </Grid>
      </Card>
              ...
              {paginatedBuses.length > 0 ? (
                paginatedBuses.map((bus: Bus) => (
                  <Table.Tr key={bus.id}>
                    ...
                    <Table.Td>
                      <Text size="sm">{bus.modelo}</Text>
                    </Table.Td>
                    <Table.Td>
                      <Text size="sm">{bus.ano}</Text>
                    </Table.Td>
                    <Table.Td>
                      <Text size="sm" fw={500}>
                        {bus.capacidade} passageiros
                      </Text>
                    </Table.Td>
                    <Table.Td>
                      <Badge
                        ...
                        variant="light"
                        leftSection={getStatusIcon(bus.status)}
                      >
                        {bus.status}
                      </Badge>
                    </Table.Td>
      ...
      <Modal
        opened={opened}
        onClose={close}
        title={editingBus ? "Editar Ônibus" : "Adicionar Ônibus"}
        size="lg"
      >
        <form
          onSubmit={(e) => {
